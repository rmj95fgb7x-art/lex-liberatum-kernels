//! KL-142-MudWeightWindow – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if mud weight outside pore/frac window
    let decision = if report.score >= 1 && report.score <= 100 { Decision::Certified } else { Decision::Rejected };

//! KL-144-TripMargin – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if trip margin < 0.5 ppg
    let decision = if report.score >= 50 { Decision::Certified } else { Decision::Rejected };

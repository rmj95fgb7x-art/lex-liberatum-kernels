//! KL-140-BlowoutPreventerTest – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if BOP test > 14 days old
    let decision = if report.score <= 14 { Decision::Certified } else { Decision::Rejected };

//! KL-203-DataRetention – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if > 6 yrs unless statute
    let decision = if report.score <= 6 { Decision::Certified } else { Decision::Rejected };

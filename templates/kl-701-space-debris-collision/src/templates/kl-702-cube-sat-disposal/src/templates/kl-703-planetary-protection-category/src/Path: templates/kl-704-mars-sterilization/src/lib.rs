//! KL-704-MarsSterilization – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if sterilization spores > 30
    let decision = if report.score <= 30 { Decision::Certified } else { Decision::Rejected };

//! KL-709-CrewRadiationDose – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if crew radiation > 500 mSv/month
    let decision = if report.score <= 500 { Decision::Certified } else { Decision::Rejected };

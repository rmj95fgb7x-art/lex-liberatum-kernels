//! KL-312-AntennaHeight – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if antenna height > 300 m AGL
    let decision = if report.score <= 300 { Decision::Certified } else { Decision::Rejected };

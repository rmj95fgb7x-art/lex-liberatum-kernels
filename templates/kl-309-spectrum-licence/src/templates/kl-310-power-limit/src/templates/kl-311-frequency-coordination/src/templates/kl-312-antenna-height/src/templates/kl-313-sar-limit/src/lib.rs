//! KL-313-SARLimit – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if SAR > 1.6 W/kg
    let decision = if report.score <= 16 { Decision::Certified } else { Decision::Rejected };

//! KL-315-EMFWorkerExposure – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if EMF exposure > 200 µT
    let decision = if report.score <= 200 { Decision::Certified } else { Decision::Rejected };

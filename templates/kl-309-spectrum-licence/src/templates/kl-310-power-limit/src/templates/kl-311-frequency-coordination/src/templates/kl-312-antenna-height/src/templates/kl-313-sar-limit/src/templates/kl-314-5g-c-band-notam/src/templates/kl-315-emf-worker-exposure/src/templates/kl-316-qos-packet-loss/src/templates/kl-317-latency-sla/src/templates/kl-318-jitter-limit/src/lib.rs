//! KL-318-JitterLimit – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if jitter > 5 ms
    let decision = if report.score <= 5 { Decision::Certified } else { Decision::Rejected };

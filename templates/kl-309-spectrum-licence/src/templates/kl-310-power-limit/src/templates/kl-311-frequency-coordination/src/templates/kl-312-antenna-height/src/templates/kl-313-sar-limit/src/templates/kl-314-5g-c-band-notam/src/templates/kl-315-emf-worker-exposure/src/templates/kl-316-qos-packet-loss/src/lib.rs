//! KL-316-QoSPacketLoss – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if packet loss > 0.1 %
    let decision = if report.score <= 10 { Decision::Certified } else { Decision::Rejected };

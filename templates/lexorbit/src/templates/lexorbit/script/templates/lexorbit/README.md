# LexOrbit – Satellite Maneuver Logger (FCC Compliance)

## What it does
- Accepts NORAD ID + burn duration (ms) + delta-v (m/s) + timestamp
...
2. Deploy adapter: `forge script script/DeployLexOrbit.sol --rpc-url $BASE_RPC --private-key $PK --broadcast`
3. Call `logManeuver(satId, burnMs, deltaV)` – royalty appears instantly.

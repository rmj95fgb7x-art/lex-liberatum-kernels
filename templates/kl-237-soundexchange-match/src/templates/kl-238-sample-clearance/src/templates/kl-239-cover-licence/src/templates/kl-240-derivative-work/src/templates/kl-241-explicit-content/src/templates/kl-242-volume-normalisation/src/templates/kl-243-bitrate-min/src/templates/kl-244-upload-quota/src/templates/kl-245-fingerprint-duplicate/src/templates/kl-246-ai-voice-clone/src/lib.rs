//! KL-246-AIVoiceClone – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if voice-print match > 0.90
    let decision = if report.score <= 90 { Decision::Certified } else { Decision::Rejected };

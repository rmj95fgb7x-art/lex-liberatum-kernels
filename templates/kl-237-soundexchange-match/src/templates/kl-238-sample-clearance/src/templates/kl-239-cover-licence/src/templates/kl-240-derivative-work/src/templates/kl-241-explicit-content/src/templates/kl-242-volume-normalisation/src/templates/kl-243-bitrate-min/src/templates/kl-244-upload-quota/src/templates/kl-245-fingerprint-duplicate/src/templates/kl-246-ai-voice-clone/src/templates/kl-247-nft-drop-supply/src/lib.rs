//! KL-247-NFTDropSupply – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if > 10 k editions
    let decision = if report.score <= 10_000 { Decision::Certified } else { Decision::Rejected };

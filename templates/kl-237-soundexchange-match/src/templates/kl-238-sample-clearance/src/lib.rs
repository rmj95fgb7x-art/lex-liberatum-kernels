//! KL-238-SampleClearance – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if sample > 15 sec not cleared
    let decision = if report.score <= 15 { Decision::Certified } else { Decision::Rejected };

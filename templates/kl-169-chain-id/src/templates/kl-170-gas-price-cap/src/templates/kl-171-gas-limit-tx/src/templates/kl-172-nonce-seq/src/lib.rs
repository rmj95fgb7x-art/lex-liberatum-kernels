//! KL-172-NonceSeq – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if nonce ≠ prev+1
    let prev = seq - 1;
    let decision = if report.score == prev { Decision::Certified } else { Decision::Rejected };

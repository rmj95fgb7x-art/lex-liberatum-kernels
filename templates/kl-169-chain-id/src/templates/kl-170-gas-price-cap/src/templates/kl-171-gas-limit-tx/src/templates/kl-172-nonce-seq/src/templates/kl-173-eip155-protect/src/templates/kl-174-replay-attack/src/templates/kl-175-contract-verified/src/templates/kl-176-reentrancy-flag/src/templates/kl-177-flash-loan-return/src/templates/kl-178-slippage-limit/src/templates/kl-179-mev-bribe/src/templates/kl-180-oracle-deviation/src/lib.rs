//! KL-180-OracleDeviation – copyright 2025 Lex Libertatum Trust – Per-Decision Royalty Licence
...
pub fn decide(report: &Report) -> (Decision, Certificate) {
    ...
    // DENY if price oracle deviation > 1 %
    let decision = if report.score <= 100 { Decision::Certified } else { Decision::Rejected };
